...
class ProjectState extends State {
    ...
    updateListeners() {
        for (const listenerFn of this.listeners) {
            listenerFn(this.projects.slice()); // deep copy of array
        }
    }
}
...
function validate(validatableInput) {
    ...
    if (validatableInput.minLength != null && // truthy has problems with setting to 0?
...
class ProjectList extends Component {
    constructor(type) {
        ...
        // safer to call these in the inherited classes even though they're required
        this.configure();
        this.renderContent();
    }
    dragOverHandler(event) {
        if (event.dataTransfer && event.dataTransfer.types[0] === "text/plain") {
            event.preventDefault(); // default not allowed drag & drop
